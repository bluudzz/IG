...
const User = [
  {
    type:'input',
    name:'username',
    message:'Lebokna Username'
  },
  {
    type:'password',
    name:'password',
    message:'Lebokna Password',
    mask:'*'
  },
  {
    type:'input',
    name:'target',
    message:'Targete Sapa (tanpa @[at])'
  },
  {
    type:'input',
    name:'text',
    message:'Tulis komentare sampean 1 (Gunakan Pemisah [|] bila lebih dari 1)'
  },
  {
    type:'input',
    name:'sleep',
...
const CommentAndLike = async function(session, accountId, text){
  ...
  try {
    ...
    if (result.length > 0) {
      ...
      return chalk`{bold.green BERHASIL BOS [Follow,Comment,Like]} | ${text}`;
    }
    return chalk`{bold.green BERHASIL BOS [FOLLOW]}`
  } catch (err) {
    // console.log(err);
      ...
      for (let i = 0; i < TargetResult.length; i++) {
        await Promise.all(TargetResult[i].map(async(akun) => {
          if (!getFollowers.includes(akun.id) && akun.params.isPrivate === false) {
            ...
            console.log(chalk`{bold.yellow [SKIPPED]}${akun.params.username} => PRIVATE mbuh WIS DIFOLLOW`)
          }
        }));
...
{green KOJAWA TV - KOPER JAYA WLAHAR - Maswin}
{bold.red Code By Ccocot | ReCode by Maswin}
`);
